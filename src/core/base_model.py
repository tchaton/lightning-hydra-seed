        ...
        if optimizers_conf is not None:
            for optim_conf in optimizers_conf:
                if name == optim_conf["name"]:
                    ...
                    optim_conf = OmegaConf.to_container(optim_conf, resolve=True)
                    delete_key(optim_conf, "name")
                    delete_key(optim_conf, "_target_")
                    return optim_cls([p for p in params], **optim_conf)
            raise Exception(
